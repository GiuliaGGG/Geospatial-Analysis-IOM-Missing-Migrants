...
# Define icon 
icons <- makeIcon(iconUrl = "person-circle-question-solid.png", iconWidth = 18, iconHeight = 18)

# Define UI with the new theme
ui <- fluidPage(
  theme = bslib::bs_theme(bootswatch = "darkly"),
  
  tags$head(
    tags$link(rel = "stylesheet", type = "text/css", href = "style.css"),
    tags$title("Global Missing Migrants Map")
  ),
  
  div(class = "container",
      tags$h1("Missing Migrants: Global Overview"),
      tags$h2("Interactive Map and Statistics"),
      
      div(class = "row",
          div(class = "col-md-4",
              tags$section(
                selectInput("category", "Select Geographical Grouping", 
                            choices = c("None", unique(mm_df$UNSD.Geographical.Grouping))),
                selectInput("year", "Select Year", 
                            choices = c("None", unique(mm_df$Incident.year))),
                selectInput("month", "Select Month of Year", 
                            choices = c("None", unique(mm_df$Reported.Month))),
                plotOutput("incidents_plot", height = "400px")
              )
          ),
          div(class = "col-md-8",
              tags$section(
...
server <- function(input, output, session) {
  ...
  output$worldmap <- renderLeaflet({
    ...
    leaflet(filtered_data()) %>% 
      ...
      addMarkers(
        ~lon, ~lat, icon = icons, label = labels,
  ...
  output$incidents_plot <- renderPlot({
    ...
    ggplot(incidents_per_mr, aes(x = Migration.route, y = Incidents, fill = Incidents)) +
      ...
      labs(title = "Incidents per Migration Route",
           x = "Migration Routes", y = "Number of Incidents (Log Scale)") +
      scale_y_log10() +
      theme(
